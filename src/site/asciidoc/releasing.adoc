...
Or non-interactively: `release:prepare -DreleaseVersion=1.2.0 -DdevelopmentVersion=1.3.0-SNAPSHOT -Dtag=v1.2.0`.
...
Central. Tests are not re-run (`-DskipTests`); `release:prepare` already verified them.

. Confirm the artifact at
https://central.sonatype.com/artifact/io.github.database-audits/database-audits-core[Maven Central]. `main` now
carries the next `-SNAPSHOT`, and `publish-docs.yml` redeploys the site after the following `main` build.
