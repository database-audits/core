...
**`SchemaEntityValidationAudit`** takes an `EntityManagerFactory`. When `hibernate.hbm2ddl.auto=validate` is
active, Hibernate validates every mapped entity at factory-build time; if the factory was built, validation
passed. The audit checks the factory is non-null as a formality confirming the paired test enabled validation.
